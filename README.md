...
每一对页面的第 13 节都测格式的精度阶，网格族都由基础网格逐次一致加密得到。伴随给出的是
...
Section 13 of every pair measures the scheme's order, on a mesh family refined uniformly from the
...
- **1-D Burgers**: no manufactured solution is needed, the exact one being
  ...
  $\lambda(x)=\int_x^1(u-\bar u)/u\,\mathrm dt$ the discrete one is first order inside, but its error in
